...
- **커밋 시점 2**: 완료 (`[Cycle 15][GREEN+REVIEW]`, commit 4a2d05f, 푸쉬 완료).
  Cycle 15 종료. **system.pdf 재검토(Cycle 13~15) 로드맵 전체 완료.**

## 이력 (History) — 2차 재검토

### [2026-07-15] system.pdf 2차 정독 + 코드 전수 재검토

- **배경**: 사용자 요청으로 Cycle 13~15 반영 이후 `system.pdf`를 다시 처음부터
  정독하며 전체 소스 코드(model/repository/service/view/controller/main.py, 25개
  파일 전부)를 새로 읽고 재대조.
- **점검 내용**:
  - PDF 전 페이지(배경/역할/시스템개요/주문상태표/메인메뉴/시료관리/시료주문/승인거절/
    모니터링/생산라인/출고처리)를 항목별로 코드와 재대조 — Cycle 13~15에서 수정한
    3건 모두 실제 코드에 정확히 반영되어 있음을 확인.
  - `grep`으로 `ApprovalController`/`ShipmentController` 생성부와 `order.sample_id`
    직접 출력 잔존 여부를 전수 검색 — 누락된 연동/스테일 참조 없음.
  - 전체 테스트(`pytest`) 50 passed, `ruff check` All checks passed 재확인.
  - 콘솔 전체 메뉴(시료 등록→목록→검색→주문→승인 미리보기→생산라인→생산완료→모니터링
    →출고→종료)를 한 세션에서 끝까지 실제 실행 — 크래시 없이 정상 동작, 시료명 표시와
    재고 확인 미리보기 모두 정상 노출됨을 확인.
  - `examples/seed_example_db.py` 재실행 확인, `examples/example.db`는 커밋된 상태와
    diff 없음(깨끗한 상태 유지).
- **결과**: **새로 발견된 미구현/버그 없음.** 기존에 의도적으로 보류한 항목만 잔존:
  - 시료 검색은 이름만 지원(ID 미지원) — PRD.md에 이미 백로그로 명시.
  - 타임스탬프/처리일시 필드 없음 — 기능명세엔 명시적 요구사항 아님, 우선순위 낮음.
  - 시료 등록 시 "재고" 입력 자체가 PDF 12페이지 속성값 표엔 없음 — 이미 문서화된
    해석/가정.
- **상태**: 완료.
...
## 진행 중 (Active) — README.md 재검토

### README.md 재검토 및 수정

- **배경**: 사용자 요청으로 README.md가 현재 코드 상태(Cycle 0~15 반영분)와 실제로
  맞는지 검토.
- **발견한 문제**:
  1. **"데이터 영속성" 섹션이 오래된 내용** — "생산 큐는 콘솔 세션 동안만 메모리에
     유지되며 재시작 시 초기화됨"이라고 되어 있으나, Cycle 9~12에서 정확히 이 문제를
     해결해 이제는 `production_jobs` 테이블에 영속화되고 재시작 시 복원됨. 실제와
     반대되는 설명이라 시급히 수정 필요.
  2. **"기본 사용 흐름" 3번(승인/거절)에 재고 확인 미리보기(Cycle 14) 언급 없음.**
  3. **"프로젝트 구조"에 `PoC/`가 저장소에 포함된 것처럼 표기됨** — 실제로는
     `.gitignore`의 `/PoC` 규칙으로 추적되지 않아, 새로 클론하면 없는 폴더. 오해 소지.
  4. **"프로젝트 구조" 트리에 `examples/`가 빠져 있음** — 실제로는 git에 추적되는
     최상위 폴더인데 트리 다이어그램엔 없음.
  5. **"린트" 섹션 명령이 실제 작업 흐름에서 쓰는 명령과 다름** — 저장소 전체에서
     쓰는 `ruff check` 대상에 `examples/seed_example_db.py`가 빠져 있음.
  6. **시료 등록 안내에 수율/생산시간 검증(Cycle 13) 언급 없음.**
- **수정 내용**: 위 6건 모두 README.md에 반영.
- **TDD 해당 없음**: 문서 수정, SKILL.md의 문서 예외 대상.
- **상태**: 완료. README.md에 6건 모두 반영, 문서에 적힌 명령(ruff, pytest) 재실행으로 정확성 확인 완료.

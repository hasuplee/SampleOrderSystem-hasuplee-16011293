...
## 이 저장소의 워크플로우: Human-in-the-loop RED → GREEN → REVIEW

이 저장소에서 새 기능/버그 수정을 진행할 때는 아래 3단계를 순서대로 따른다. 각 단계는 독립된 작업 단위이며, **커밋은 정해진 두 시점에서만, 그리고 매번 사람 파트너의 명시적 확인을 받은 뒤에만** 실행한다. 확인 없이 임의로 `git commit`을 실행하지 않는다.

### 1) RED — 목표 설정 + Plan.md

- 이번에 구현할 목표(goal)를 정확하고 좁게 정의한다 (모호하면 사람 파트너에게 먼저 질문한다).
- `Plan.md`를 작성한다. 최소한 다음을 포함한다:
  - 목표(goal): 이번 사이클에서 만족시킬 요구사항
  - 범위(scope): 포함하는 것과 명시적으로 포함하지 않는 것
  - 테스트 계획: 작성할 실패 테스트 목록(함수명 포함, 한글 네이밍 규칙 준수)
- Plan.md를 사람 파트너에게 제시하고 **검토/승인을 받는다.** 승인 전에는 다음 단계로 넘어가지 않는다.
- 승인받은 계획대로 실패하는 테스트를 작성하고, 표준 RED 검증(아래 "RED 검증" 절)을 수행해 제대로 실패하는지 확인한다.
- **커밋 시점 1**: RED 단계가 끝나면 `Plan.md`를 커밋 대상으로 스테이징한다. **커밋을 실행하기 전 반드시 사람 파트너에게 커밋 여부를 확인받는다.** 승인받으면 `Plan.md`만(또는 Plan.md와 이번에 작성한 실패 테스트를 함께, 사람 파트너가 정한 범위대로) 커밋한다.
...
- **커밋 시점 2**: REVIEW가 끝나면 GREEN 단계에서 만들어진(그리고 REVIEW에서 정리된) 코드를 커밋 대상으로 스테이징한다. **커밋을 실행하기 전 반드시 사람 파트너에게 커밋 여부를 확인받는다.** 승인받은 뒤에만 커밋한다.

이 워크플로우 밖에서, 즉 RED 종료 시점과 REVIEW 종료 시점이 아닌 다른 시점에 커밋을 제안하거나 실행하지 않는다.

커밋 메시지 형식(사이클 번호, RED/REVIEW 각각의 제목·본문 템플릿)은 `COMMIT_CONVENTION.md`를 따른다. 이 형식을 지키면 커밋 로그만으로 각 TDD 사이클의 목표 → 계획 → 구현 → 리뷰 흐름을 재구성할 수 있다.
...
- [ ] 두 커밋 시점(RED 종료, REVIEW 종료) 모두 커밋 전 사람 파트너의 확인을 받았다

...
def _서비스(sample_repo, order_repo, production_queue=None):
    return ApprovalService(sample_repo, order_repo, production_queue or ProductionQueue())


def test_주문을_거절하면_REJECTED_상태로_변경된다(tmp_db_path):
    sample_repo, order_repo, order = _주문_준비(tmp_db_path)
    service = _서비스(sample_repo, order_repo)
...
def test_재고가_부족하면_승인시_생산_큐에_작업이_등록된다(tmp_db_path):
    ...
    service = _서비스(sample_repo, order_repo, queue)

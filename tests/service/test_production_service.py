...
def _생산중인_주문_준비(tmp_db_path, stock=10, quantity=30, avg_production_time=0.5, yield_rate=0.92):
    ...
    return sample_repo, order_repo, queue, job_repo, order


def test_생산완료_처리하면_재고가_실생산량만큼_증가한_뒤_주문수량만큼_차감된다(tmp_db_path):
    sample_repo, order_repo, queue, job_repo, order = _생산중인_주문_준비(
        tmp_db_path, stock=10, quantity=30, avg_production_time=0.5, yield_rate=0.92,
    )
    service = ProductionService(sample_repo, order_repo, queue, job_repo)
...
def test_생산완료_처리하면_주문_상태가_CONFIRMED로_변경된다(tmp_db_path):
    sample_repo, order_repo, queue, job_repo, order = _생산중인_주문_준비(tmp_db_path)
    service = ProductionService(sample_repo, order_repo, queue, job_repo)
...
def test_생산완료_처리하면_저장소에서도_작업이_제거된다(tmp_db_path):
    sample_repo, order_repo, queue, job_repo, order = _생산중인_주문_준비(tmp_db_path)
    service = ProductionService(sample_repo, order_repo, queue, job_repo)
